...
if [ -z "${ECS_SQUID_CONFIG_S3_BUCKET}" -o -z "${ECS_SQUID_CONFIG_S3_PREFIX}" ]; then
    echo "ERROR: ECS_SQUID_CONFIG_S3_BUCKET and ECS_SQUID_CONFIG_S3_PREFIX environment variables must be provided"
    exit 1
fi

S3_URI="s3://${ECS_SQUID_CONFIG_S3_BUCKET}/${ECS_SQUID_CONFIG_S3_PREFIX}"
...
exec $(which squid) -f /etc/squid/squid.conf -NYCd 1 ${ECS_SQUID_EXTRA_ARGS}
